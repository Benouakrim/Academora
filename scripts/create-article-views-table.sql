...
-- Create function to update article view count (only on INSERT, not UPDATE)
CREATE OR REPLACE FUNCTION update_article_view_count()
RETURNS TRIGGER AS $$
BEGIN
  -- Only increment on INSERT operations (new views)
...
DROP TRIGGER IF EXISTS trigger_update_article_view_count ON article_views;
